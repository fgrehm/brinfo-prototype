...
var _ = Describe("ScrapeArticleContent", func() {
	var (
	...
	Context("validations", func() {
		It("fails if no url provided", func() {
			_, err := ScrapeArticleContent(ScrapeArticleContentInput{
				Url:  "",
				Repo: mem.NewContentSourceRepo(),
			})
		...
		It("fails if no content source and repo are provided", func() {
			_, err := ScrapeArticleContent(ScrapeArticleContentInput{Url: ts.URL})
			Expect(err).To(HaveOccurred())
	...
	Context("extraction", func() {
		...
		It("gets delegated to ContentSource article scraper", func() {
			data, err := ScrapeArticleContent(ScrapeArticleContentInput{
				Url:           ts.URL + "/good",
				ContentSource: cs,
			})
